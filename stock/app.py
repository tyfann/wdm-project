...
def create_item(price: int):
    while True:
        ...
        response = cni.query("INSERT INTO stock (item_id, unit_price, stock_amount) VALUES (%s,%s, 0) RETURNING item_id",
...
def add_stock(item_id: str, amount: int):
    return cni.get_status("UPDATE stock SET stock_amount = stock_amount + %s WHERE item_id=%s AND stock_amount + %s > stock_amount",
...
def remove_stock(item_id: str, amount: int):
    return cni.get_status("UPDATE stock SET stock_amount = stock_amount - %s WHERE item_id=%s AND stock_amount - %s >= 0",
                          [amount, item_id, amount], g.connection)
